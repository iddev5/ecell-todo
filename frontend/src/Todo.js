...
function Todo({data, todos, setTodos}) {
	...
	return <div className="list-group-item">
	      ...
	      <div className="container">
					<div className="row">
					<button className="col-1 p-0" onClick={markCompleted} checked></button>
	        <button ref={titleRef} className="col text-start border-0 bg-transparent" onClick={() => setChange(true)}><h1>{data.title}</h1></button>
					<button className="col-1 btn bg-danger p-2 btn-close opacity-100" onClick={deleteTodo}></button>
					</div>
	        <div className="row">
						<div className="col-1"></div>
						<button ref={descRef} className="col text-start border-0 bg-transparent" onClick={() => setChange(true)}><p>{data.desc}</p></button>
					</div>
